...
export const useMapToCylinder = (
  config: Omit<Config, "config"> & {
    ...
    (progress: number) => {
      ...
      targets.map((item, idx) => {
        ...
        const transformedDistanceFromMarker =
          distanceFromMarker *
          (arrayLength / resolvedConfig.config.visibleRangeFactor);
        /*
      ...
      */
        ...
        const angle =
          (idx / relevantArrayLength) * resolvedConfig.config.availableRadians -
          offsetAngle;
        const y = Math.sin(angle) * resolvedConfig.config.cylinderRadius;
        const z =
          Math.cos(angle) * resolvedConfig.config.cylinderRadius -
          resolvedConfig.config.cylinderRadius;
        ...
        resolvedConfig.onUpdate?.(item, {
          ...
          opacity:
            transformedProgress * resolvedConfig.config.opacityDiminishFactor,
        });
      });
    },
